...
(async function () {
  ...
  // Create coursePack with known ID
  ...
  const courseList = await Promise.all(
    courses.map(async (courseFileName, index) => {
      ...
      console.log(`创建: id-${courseId} order-${index + 1} title-${convertToChineseNumber(courseName)}`);
  ...
  await Promise.all(
    courseList.map(async (course) => {
      ...
      const statementInsertTask = statementList.map(async (statement, idx) => {
