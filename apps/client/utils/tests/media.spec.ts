...
describe("getYouTubeEmbedUrl", () => {
  it("normalizes supported YouTube addresses to a trusted embed URL", () => {
    expect(getYouTubeEmbedUrl("https://www.youtube.com/watch?v=abc_123-xy")).toBe(
      "https://www.youtube-nocookie.com/embed/abc_123-xy?autoplay=1&rel=0&playsinline=1",
    );
    expect(getYouTubeEmbedUrl("https://youtu.be/abc_123-xy")).toBe(
      "https://www.youtube-nocookie.com/embed/abc_123-xy?autoplay=1&rel=0&playsinline=1",
    );
    expect(getYouTubeEmbedUrl("https://youtube.com/embed/abc_123-xy")).toBe(
      "https://www.youtube-nocookie.com/embed/abc_123-xy?autoplay=1&rel=0&playsinline=1",
    );
  });

  it("supports extracting video ids and enabling the iframe JS API", () => {
    expect(getYouTubeVideoId("https://youtu.be/abc_123-xy")).toBe("abc_123-xy");
    expect(
      getYouTubeEmbedUrl("https://youtu.be/abc_123-xy", {
        autoplay: false,
        enableJsApi: true,
        origin: "http://localhost:3000",
      }),
    ).toBe(
      "https://www.youtube-nocookie.com/embed/abc_123-xy?autoplay=0&rel=0&playsinline=1&enablejsapi=1&origin=http%3A%2F%2Flocalhost%3A3000",
    );
  });
...
describe("getMediaPlaybackUrl", () => {
  it("serves local media and trusted YouTube sources only", () => {
    expect(getMediaPlaybackUrl("courses/lesson-1.avi")).toBe(
      "/api/backend/media/stream?path=courses%2Flesson-1.avi",
    );
    expect(getMediaPlaybackUrl("https://youtu.be/abc_123-xy")).toBe(
      "https://www.youtube-nocookie.com/embed/abc_123-xy?autoplay=1&rel=0&playsinline=1",
    );
...
describe("getLocalMediaStreamUrl", () => {
  it("allows locally proxied media but rejects every remote media source", () => {
    expect(getLocalMediaStreamUrl("courses/audio-1.mp3", "&v=mp4")).toBe(
      "/api/backend/media/stream?path=courses%2Faudio-1.mp3&v=mp4",
    );
