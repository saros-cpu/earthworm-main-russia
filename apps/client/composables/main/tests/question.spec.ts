...
import { isWord, splitDisplayTokens, useInput } from "../question";
...
describe("question", () => {
  ...
  it("should ignore punctuation in the standard answer", async () => {
    const setInputCursorPosition = () => {};
    const getInputCursorPosition = () => 0;
    ...
    initialize();
    setInputValue("я хочу чай");

    const correctCallback = vi.fn();
    const wrongCallback = vi.fn();
    submitAnswer(correctCallback, wrongCallback);

    expect(userInputWords[2].text).toBe("чай");
    expect(userInputWords[2].displayText).toBe("чай.");
    expect(correctCallback).toBeCalled();
    expect(wrongCallback).not.toBeCalled();
  });
...
describe("splitDisplayTokens", () => {
  it("should render punctuation as independent tokens", () => {
    expect(splitDisplayTokens("вы говорите по-русски?")).toEqual([
      { type: "word", text: "вы", wordId: 0 },
      { type: "word", text: "говорите", wordId: 1 },
      { type: "word", text: "по-русски", wordId: 2 },
      { type: "punctuation", text: "?" },
    ]);
  });

  it("should keep comma and full stop outside word input slots", () => {
    expect(splitDisplayTokens("Спасибо, Анна.")).toEqual([
      { type: "word", text: "Спасибо", wordId: 0 },
      { type: "punctuation", text: "," },
      { type: "word", text: "Анна", wordId: 1 },
      { type: "punctuation", text: "." },
    ]);
  });
});
...
describe("isWord", () => {
  ...
  it("should return false for a string without any letters or numbers", () => {
