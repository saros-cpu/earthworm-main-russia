...
export function splitAnswerToken(content: string) {
  const prefix = content.match(/^[^\p{L}\p{N}]+/u)?.[0] || "";
  const suffix = content.match(/[^\p{L}\p{N}]+$/u)?.[0] || "";
  const text = answerTokenText(content);
...
export function splitDisplayTokens(source: string | undefined): DisplayToken[] {
  ...
  parts.forEach((part, index) => {
    ...
    const token = splitAnswerToken(part);
    if (token.prefix) {
      tokens.push({ type: "punctuation", text: token.prefix });
    }
    if (token.text) {
      tokens.push({ type: "word", text: token.text, wordId: index });
    }
    if (token.suffix) {
      tokens.push({ type: "punctuation", text: token.suffix });
    }
  });
  return tokens;
}
...
export function useInput({
  ...
  function createWord(word: string, id: number) {
    const token = splitAnswerToken(word);
    return reactive({
      text: token.text,
      displayText: word,
      isActive: false,
