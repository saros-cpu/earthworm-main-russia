<template>
  <div class="flex h-screen overflow-hidden bg-slate-50 dark:bg-slate-950">
    ...
    >
      <!-- Logo -->
      <div
        class="flex h-16 items-center gap-3 border-b border-slate-200 px-5 dark:border-slate-800"
      >
        <img
          src="/logo-circle.png"
          alt="logo"
      ...
      <div class="border-t border-slate-200 p-4 dark:border-slate-800">
        <div class="flex items-center gap-3">
          <div
            class="flex h-8 w-8 items-center justify-center rounded-full bg-purple-100 text-sm font-bold text-purple-700 dark:bg-purple-950 dark:text-purple-200"
          >
            {{ userInitial }}
          </div>
          ...
          <button
            class="rounded p-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300"
            @click="handleLogout"
            title="退出登录"
          >
            <UIcon
    ...
    <div class="flex min-w-0 flex-1 flex-col">
      <!-- 顶栏 -->
      <header
        class="flex h-16 items-center gap-4 border-b border-slate-200 bg-white px-4 dark:border-slate-800 dark:bg-slate-900"
      >
        <button
        ...
        <NuxtLink
          to="/"
          class="flex items-center gap-2 text-sm text-slate-400 hover:text-slate-600 dark:hover:text-slate-300"
        >
          <UIcon
