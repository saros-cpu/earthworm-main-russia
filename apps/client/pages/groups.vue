  ...
  <div class="w-full py-6">
    ...
    >
      <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        <button
          class="inline-flex h-11 items-center rounded-md bg-slate-950 px-4 text-sm font-bold text-white transition hover:bg-slate-800 dark:bg-white dark:text-slate-950"
    ...
    >
      ...
      <div class="space-y-3">
        ...
        <textarea
          ...
          placeholder="小组介绍（可选）"
          rows="2"
          class="w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-emerald-400 dark:border-slate-700 dark:bg-slate-800"
        ></textarea>
        <div class="flex gap-2">
          ...
          <button
            class="rounded-md border border-slate-200 px-4 py-2 text-sm text-slate-600 hover:bg-slate-50"
    ...
    <section
      v-if="allGroups.length === 0 && myGroups.length === 0 && !showCreate"
...
async function createGroup() {
  ...
  try {
    const http = getHttp();
    const group = await http<any>("/groups", {
      method: "post",
      body: { name: newGroupName.value, description: newGroupDesc.value },
    });
