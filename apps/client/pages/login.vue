<template>
  <div class="mx-auto flex min-h-[60vh] w-full max-w-md items-center justify-center py-10">
    <div
      class="w-full rounded-md border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900"
    >
      <h1 class="mb-1 text-2xl font-black text-slate-950 dark:text-white">
        {{ isLogin ? "登录" : "注册" }}
      </h1>
      ...
      <div
        v-if="error"
        class="mb-4 rounded-md bg-red-50 p-3 text-sm font-bold text-red-600 dark:bg-red-950 dark:text-red-300"
      >
        {{ error }}
      </div>
