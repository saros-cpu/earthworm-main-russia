    ...
    >
      ...
      >
        <div class="mb-2 flex items-start justify-between gap-3">
          ...
          <button
            class="shrink-0 rounded-md bg-emerald-100 px-3 py-1.5 text-xs font-bold text-emerald-700 transition hover:bg-emerald-200 dark:bg-emerald-900 dark:text-emerald-300"
...
onMounted(async () => {
  try {
    ...
    for (const r of due) {
      try {
        const course = await http<any>(
          "/course-pack/" + r.coursePackId + "/courses/" + r.courseId,
          { method: "get" },
        );
