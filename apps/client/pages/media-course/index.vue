<template>
  <div class="flex w-full flex-col py-6">
    ...
    <template v-else>
      <nav class="mb-6 flex flex-wrap gap-2">
        <button
          ...
          :class="[
            ...
            activeSeries === series.key ? series.accent.active : series.accent.chip,
          ]"
          ...
          <span
            :class="[
              ...
              activeSeries === series.key
                ? 'bg-white/30'
                : 'bg-white/70 text-slate-700 dark:bg-slate-900/70 dark:text-slate-200',
            ]"
          >
            {{ series.packs.length }}
          </span>
      ...
      >
        <header
          class="flex flex-wrap items-end justify-between gap-2 border-b border-slate-200 pb-3 dark:border-slate-800"
        >
          <div class="flex items-center gap-3">
            <span
              :class="[
                'inline-flex h-9 w-9 items-center justify-center rounded-md',
                currentSeries.accent.bubble,
              ]"
            >
              <UIcon
                :name="currentSeries.icon"
                class="h-5 w-5"
              />
            </span>
            <div>
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
          ...
          >
            <div
              class="relative aspect-video w-full overflow-hidden bg-gradient-to-br from-emerald-500 to-emerald-800"
            >
              <NuxtImg
                :src="pack.cover"
                :alt="pack.title"
            ...
            <div class="flex flex-1 flex-col p-4">
              <h3 class="font-bold text-slate-950 dark:text-white">
                <UIcon
                  v-if="packHasVideo(pack.id)"
                  name="i-ph-video"
                  class="mr-1 inline h-4 w-4 text-emerald-500"
                />
                {{ pack.title }}
              </h3>
              <p
                class="mt-1 line-clamp-2 flex-1 text-xs leading-5 text-slate-500 dark:text-slate-400"
              >
                {{ pack.description }}
              </p>
...
function classifyMedia(pack: CoursePacksItem): string {
  ...
  if (
    id.startsWith("ru-basic-") ||
    id === "ru-zero-basic" ||
    id === "ru-basic-pack" ||
    id === "ru-pronunciation-course"
  )
    return "foundation";
...
const groupedSeries = computed<MediaSeries[]>(() => {
  ...
  const series: MediaSeries[] = [
    ...
    },
    {
      ...
      label: "语法学习",
      description:
        "名词变格、动词变位、时态、前置词、成语谚语等语法与表达课程（含 A1/A2 视频素材）。",
      icon: "i-ph-pencil",
