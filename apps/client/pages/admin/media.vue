  ...
  <div class="space-y-6">
    ...
    >
      <div class="border-b border-slate-200 px-5 py-4 dark:border-slate-800">
        <div class="flex items-center gap-3">
          ...
          <span class="text-xs text-slate-400"
            >{{ filteredPacks.length }} / {{ packs.length }} 包</span
          >
        </div>
      </div>
      ...
      >
        ...
        >
          <button
            class="flex w-full items-center justify-between px-5 py-3 text-left hover:bg-slate-50 dark:hover:bg-slate-800/50"
            @click="togglePack(pack.id)"
          >
            <div class="flex items-center gap-3">
              <UIcon
                :name="expandedPacks.has(pack.id) ? 'i-ph-caret-down' : 'i-ph-caret-right'"
          ...
          <div
            v-if="expandedPacks.has(pack.id)"
            class="border-t border-slate-100 bg-slate-50/50 px-5 py-3 dark:border-slate-800 dark:bg-slate-900/50"
          >
            <div
              v-if="loadingCourses.has(pack.id)"
              class="py-4 text-center text-sm text-slate-400"
            >
              加载中...
            </div>
            <div
              v-else-if="!packCourses[pack.id]"
              class="py-4 text-center text-sm text-slate-400"
            >
              <button
                class="text-purple-600 hover:text-purple-700"
                @click="loadPackCourses(pack.id)"
              >
                点击加载课程
              </button>
            </div>
            ...
            >
              <div
                v-for="course in packCourses[pack.id]"
                :key="course.id"
                class="flex items-center gap-3 rounded-md border border-slate-200 bg-white px-4 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-900"
              >
                ...
                >
                  <span
                    class="rounded bg-emerald-50 px-2 py-0.5 text-xs font-bold text-emerald-700 dark:bg-emerald-950 dark:text-emerald-200"
                  >
                    <UIcon
                      :name="isVideo(course.video) ? 'i-ph-video' : 'i-ph-music-note'"
                      class="mr-0.5 inline h-3 w-3"
                    />
                    {{ isVideo(course.video) ? "视频" : "音频" }}
                  </span>
                  <span
                    class="max-w-[200px] truncate text-xs text-slate-400"
                    :title="course.video"
                    >{{ course.video.split("/").pop() }}</span
                  >
                </div>
                <div
                  v-else
                ...
                <button
                  class="rounded p-1 text-slate-400 hover:text-purple-600"
                  title="编辑路径"
                  @click="editCourseMedia(course)"
                >
                  <UIcon
                    name="i-ph-pencil"
                    class="h-3.5 w-3.5"
    ...
    <Teleport to="body">
      ...
      >
        <div
          class="w-[480px] rounded-xl border bg-white p-5 shadow-lg dark:border-slate-700 dark:bg-slate-900"
        >
          ...
          <input
            ...
            placeholder="媒体文件路径（如 east-uni/01/lesson1.mp3）"
          />
