    ...
    >
      ...
      >
        <div class="flex items-center gap-3">
          <div
            class="flex h-10 w-10 items-center justify-center rounded-lg bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300"
          >
            <UIcon
              name="i-ph-book-open"
              class="h-5 w-5"
          ...
          <div>
            ...
            <div class="mt-0.5 text-3xl font-black text-slate-950 dark:text-white">
              {{ stats.totals.packs }}
            </div>
          </div>
        </div>
      ...
      >
        <div class="flex items-center gap-3">
          <div
            class="flex h-10 w-10 items-center justify-center rounded-lg bg-purple-100 text-purple-700 dark:bg-purple-950 dark:text-purple-300"
          >
            <UIcon
              name="i-ph-book"
              class="h-5 w-5"
          ...
          <div>
            <div
              class="text-xs font-bold uppercase tracking-wider text-purple-700 dark:text-purple-300"
            >
              课时
            </div>
            <div class="mt-0.5 text-3xl font-black text-slate-950 dark:text-white">
      ...
      >
        <div class="flex items-center gap-3">
          <div
            class="flex h-10 w-10 items-center justify-center rounded-lg bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300"
          >
            <UIcon
          ...
          <div>
            <div
              class="text-xs font-bold uppercase tracking-wider text-amber-700 dark:text-amber-300"
            >
              练习项
            </div>
            <div class="mt-0.5 text-3xl font-black text-slate-950 dark:text-white">
              {{ stats.totals.statements.toLocaleString() }}
            </div>
          </div>
        </div>
      </article>
      ...
      >
        <div class="flex items-center gap-3">
          <div
            class="flex h-10 w-10 items-center justify-center rounded-lg bg-rose-100 text-rose-700 dark:bg-rose-950 dark:text-rose-300"
          >
            <UIcon
          ...
          <div>
            <div
              class="text-xs font-bold uppercase tracking-wider text-rose-700 dark:text-rose-300"
            >
              用户
            </div>
            <div class="mt-0.5 text-3xl font-black text-slate-950 dark:text-white">
      ...
      >
        ...
        >
          ...
          <tbody
            ...
            style="display: block; max-height: 540px; overflow-y: auto; width: 100%"
          >
            <tr
            ...
            >
              ...
              <td
                class="w-[70px] px-2 py-2 text-right font-mono text-slate-700 dark:text-slate-200"
              >
                {{ p.courses }}
              </td>
              <td
                class="w-[90px] px-2 py-2 text-right font-mono font-bold text-emerald-700 dark:text-emerald-300"
              >
                {{ p.statements.toLocaleString() }}
              </td>
...
async function reload() {
  loading.value = true;
  try {
    const [s, u] = await Promise.all([getHttp()<Stats>("/admin/stats"), fetchAdminUsers()]);
