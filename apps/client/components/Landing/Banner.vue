<template>
  ...
  >
    ...
    <div
      class="banner-bg-letters"
      aria-hidden="true"
    >
      <span
        v-for="(item, i) in BG_LETTERS"
        :key="i"
        :style="item.style"
        >{{ item.ch }}</span
      >
    ...
    <div
      class="relative mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-screen-xl items-center px-5 pb-12 pt-20"
    >
      <div class="grid w-full items-center gap-10 md:grid-cols-[minmax(0,1fr)_auto]">
        <div class="max-w-3xl">
          ...
          <p
            class="mt-5 max-w-2xl text-xl leading-9 text-slate-700 dark:text-slate-300 md:text-2xl"
          >
            像玩游戏一样学俄语。先练 33
            个西里尔字母和高频词，再进入教材句子，输入、听读、反馈一口气完成。
          </p>
...
.banner-bg-overlay {
  ...
  background: linear-gradient(
    90deg,
    rgba(251, 247, 240, 0.6) 0%,
    rgba(251, 247, 240, 0.15) 70%,
    transparent 100%
  );
  pointer-events: none;
}
:global(.dark) .banner-bg-overlay {
  background: linear-gradient(
    90deg,
    rgba(15, 18, 24, 0.65) 0%,
    rgba(15, 18, 24, 0.2) 70%,
    transparent 100%
  );
}
</style>
