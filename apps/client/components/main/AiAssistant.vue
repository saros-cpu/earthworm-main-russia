<template>
  <div class="fixed bottom-6 right-6 z-50">
    <Transition name="chat-slide">
      ...
      >
        <div
          class="flex items-center justify-between border-b border-slate-200 bg-emerald-50 px-4 py-3 dark:border-slate-700 dark:bg-emerald-950"
        >
        ...
        <div class="border-t border-slate-200 p-3 dark:border-slate-700">
          ...
          <div class="mt-1 flex justify-between text-[10px] text-slate-400">
            ...
            <span
              v-if="courseStore.currentStatement?.english"
...
async function send() {
  ...
  if (questionCount.value >= dailyLimit.value) {
    messages.value.push({
      role: "assistant",
      content: "今日提问次数已达上限（" + dailyLimit.value + "次），明天再来吧！",
    });
    return;
  }
  question.value = "";
