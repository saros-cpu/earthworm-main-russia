<template>
  <header
    ...
    :class="{
      ...
      'bg-white/88 dark:bg-slate-950/88 border-b border-slate-200 shadow-sm backdrop-blur dark:border-slate-800':
        ...
        >
          ...
          <template v-if="isAuthenticated()">
            ...
            >
              ...
              <span
                v-if="dueReviewCount > 0"
                class="ml-2 rounded-full bg-red-500 px-1.5 py-0.5 text-[10px] font-bold text-white"
                >{{ dueReviewCount }}</span
              >
            </NuxtLink>
            <NuxtLink
...
onMounted(() => {
  if (isAuthenticated()) loadDueReviews();
});
